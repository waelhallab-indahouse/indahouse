...
function LiveSession({booking, onEnd}) {
  ...
  // ── Daily.co WebRTC state ──────────────────────────────────────────────────
  const [callObj,       setCallObj]       = useState(null);
  const [djVideoTrack,  setDjVideoTrack]  = useState(null);
  const [djAudioTrack,  setDjAudioTrack]  = useState(null);
  const [hostCamOn,     setHostCamOn]     = useState(false);
  const [hostMicOn,     setHostMicOn]     = useState(false);
  const [camErr,        setCamErr]        = useState(null);
  const [streamMode,    setStreamMode]    = useState("demo"); // "demo" | "live"
  ...
  const djVidRef    = useRef(null);   // DJ video element
  const hostVidRef  = useRef(null);   // Host camera PiP
  const audioRef    = useRef(null);   // DJ audio element
  const callRef     = useRef(null);   // Daily.co call object

  const isDJ   = booking.isDJ;  // true when DJ opens the session
  const roomName = `indahouse-${booking.id || booking.dj?.id || 'demo'}`;
  ...
  // ── Demo audio engine (plays when not in real live mode) ─────────────────
  useDeepHouseEngine(audioOn && phase === "live" && streamMode === "demo", vol);

  // ── Daily.co: Join room ──────────────────────────────────────────────────
  const joinDailyRoom = async () => {
    setConnecting(true);
    try {
      // Dynamically load Daily.co SDK
      ...
      // Create Daily room via your API
      ...
      if (roomRes.ok) {
        ...
        // Fallback: use Daily.co subdomain directly
        roomUrl = `https://indahouse.daily.co/${roomName}`;
      }

      // Create call object
      const call = window.DailyIframe.createCallObject({
        audioSource: isDJ,   // DJ sends audio
        videoSource: isDJ,   // DJ sends video
      ...
      call.on('track-started', (e) => {
        if (!e.participant.local) {
          if (e.track.kind === 'video') {
            setDjVideoTrack(e.track);
            if (djVidRef.current) {
              djVidRef.current.srcObject = new MediaStream([e.track]);
            }
          }
          if (e.track.kind === 'audio') {
            setDjAudioTrack(e.track);
            if (audioRef.current) {
      ...
      call.on('track-stopped', (e) => {
        if (e.track.kind === 'video') setDjVideoTrack(null);
        if (e.track.kind === 'audio') setDjAudioTrack(null);
      });

      // Join the room
      await call.join({
        url: roomUrl,
        userName: isDJ ? 'dj' : 'host',
        audioSource: isDJ,
        videoSource: isDJ,
      });
      ...
      // If DJ: start sending audio+video
      if (isDJ) {
        await call.setLocalAudio(true);
        await call.setLocalVideo(true);
        // Show DJ's own camera
        const localVideo = call.participants().local?.videoTrack;
        if (localVideo && hostVidRef.current) {
          hostVidRef.current.srcObject = new MediaStream([localVideo]);
        }
      }
    } catch(err) {
      console.error('Daily.co error:', err);
      setCamErr(`Could not connect to stream: ${err.message}. Using demo mode.`);
      setStreamMode('demo');
      setConnecting(false);
    }
  };

  // ── Toggle host camera (for host to show party to DJ) ────────────────────
  const toggleHostCam = async () => {
    if (streamMode === 'live' && callRef.current) {
      if (hostCamOn) {
      ...
      // Demo mode — use getUserMedia
      try {
        if (hostCamOn) {
          setHostCamOn(false);
        } else {
          const stream = await navigator.mediaDevices.getUserMedia({video:true, audio:false});
          if (hostVidRef.current) hostVidRef.current.srcObject = stream;
          setHostCamOn(true);
          setCamErr(null);
        }
    ...
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{maxWidth:500,width:"100%",textAlign:"center"}}>
        ...
        <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:22,marginBottom:22,textAlign:"left"}}>
          {[["DJ",booking.dj?.name],["Event",booking.eventType],["Duration",booking.duration],["Song Requests",requests.length],["Stream Mode",streamMode==="live"?"🟢 Live via Daily.co":"🎵 Demo audio"]].map(([k,v])=>(
    ...
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:"24px 16px"}}>
      <div style={{maxWidth:560,width:"100%"}}>
        <div style={{position:"relative",borderRadius:16,overflow:"hidden",aspectRatio:"16/9",marginBottom:20,background:"#07101F",border:`2px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"center"}}>
          {djVideoTrack ? (
            <video ref={djVidRef} autoPlay playsInline muted={false} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
          ) : (
            <div style={{textAlign:"center"}}>
        ...
        <div style={{background:C.card,border:`1px solid ${C.primaryBorder}`,borderRadius:16,padding:28}}>
          ...
          <Btn onClick={startSession} full variant="green" sx={{padding:"15px",fontSize:15}}>
            {connecting ? "⏳ Connecting…" : "▶ Start Streaming Now"}
          </Btn>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#030810",color:C.text,overflow:"hidden",fontFamily:"inherit",
      ...
      {/* Hidden audio element for DJ stream */}
      <audio ref={audioRef} autoPlay playsInline style={{display:"none"}}/>

      {/* DJ video fullscreen (real stream) or canvas animation (demo) */}
      {streamMode === "live" && djVideoTrack ? (
        <video ref={djVidRef} autoPlay playsInline
      ...
      {/* Overlays */}
      <div style={{position:"absolute",inset:0,background:"rgba(3,8,16,0.35)",zIndex:2,pointerEvents:"none"}}/>
      <div style={{position:"absolute",inset:0,zIndex:3,
        background:`radial-gradient(ellipse at 50% 30%, ${C.primary}${beat?"16":"00"} 0%, transparent 55%)`,
        transition:"background 0.09s",pointerEvents:"none"}}/>

      {/* Stream mode badge */}
      <div style={{position:"absolute",top:"calc(env(safe-area-inset-top) + 60px)",left:16,zIndex:15}}>
        {streamMode === "live" ? (
          <div style={{background:"rgba(0,217,138,0.2)",border:`1px solid ${C.green}`,borderRadius:6,padding:"3px 10px",fontSize:10,color:C.green,fontWeight:800,letterSpacing:1}}>
            📡 LIVE STREAM
          </div>
        ) : (
          <div style={{background:"rgba(255,107,26,0.15)",border:`1px solid ${C.primary}44`,borderRadius:6,padding:"3px 10px",fontSize:10,color:C.primary,fontWeight:800,letterSpacing:1}}>
            🎵 DEMO MODE
          </div>
        )}
      </div>
      ...
      {/* Connection error */}
      {camErr && (
        <div style={{position:"absolute",top:"calc(env(safe-area-inset-top) + 80px)",left:16,right:16,zIndex:15,
          background:"rgba(255,59,92,0.15)",border:`1px solid ${C.red}44`,borderRadius:8,padding:"8px 12px",fontSize:11,color:C.red}}>
          {camErr}
        </div>
      )}
      ...
      {/* Host camera PiP */}
      {hostCamOn && (
        <div style={{position:"absolute",bottom:100,left:16,zIndex:20,width:130,aspectRatio:"4/3",borderRadius:12,overflow:"hidden",
      ...
      {/* Participant count */}
      {streamMode === "live" && Object.keys(participants).length > 0 && (
        <div style={{position:"absolute",bottom:100,right:16,zIndex:20,
          background:"rgba(0,0,0,0.7)",border:`1px solid ${C.border}`,borderRadius:8,padding:"6px 12px",fontSize:11,color:C.sub}}>
          👥 {Object.keys(participants).length + 1} in session
        </div>
      )}

      {/* VISUALIZER */}
      <div style={{position:"absolute",bottom:0,left:0,right:0,height:90,zIndex:4,pointerEvents:"none",
      ...
      {/* SESSION PROGRESS */}
      <div style={{position:"absolute",bottom:84,left:0,right:0,zIndex:5,padding:"0 16px"}}>
          ...
          <div style={{width:290,height:"100%",display:"flex",flexDirection:"column",overflowY:"auto"}}>
            ...
            <div style={{padding:"18px 16px 14px",borderBottom:`1px solid ${C.border}`}}>
              ...
              <div style={{display:"flex",gap:8}}>
                {/* DJ cam (shows real video or demo) */}
                <div style={{flex:1,background:streamMode==="live"&&djVideoTrack?`${C.green}18`:`${C.primary}18`,
                  border:`1px solid ${streamMode==="live"&&djVideoTrack?C.green:C.primary}44`,
                  borderRadius:8,padding:"10px 8px",textAlign:"center"}}>
                  <div style={{fontSize:18,marginBottom:3}}>🎬</div>
                  <div style={{fontSize:10,fontWeight:800,color:streamMode==="live"&&djVideoTrack?C.green:C.primary,textTransform:"uppercase",letterSpacing:1}}>DJ Cam</div>
                  <div style={{fontSize:9,color:C.sub,marginTop:2}}>{streamMode==="live"&&djVideoTrack?"Live":"Demo"}</div>
                </div>
                {/* Host cam */}
                <button onClick={toggleHostCam} style={{flex:1,
                  ...
                  <div style={{fontSize:18,marginBottom:3}}>📱</div>
                  <div style={{fontSize:10,fontWeight:800,color:hostCamOn?C.accent:C.sub,textTransform:"uppercase",letterSpacing:1}}>Your Cam</div>
                  <div style={{fontSize:9,color:C.sub,marginTop:2}}>{hostCamOn?"On":"Off"}</div>
                </button>
                {/* Host mic (only in live mode) */}
                {streamMode==="live" && (
                  <button onClick={toggleHostMic} style={{flex:1,
                    background:hostMicOn?`${C.yellow}18`:"rgba(255,255,255,0.04)",
                    border:`1px solid ${hostMicOn?C.yellow:C.border}`,
                    borderRadius:8,padding:"10px 8px",cursor:"pointer",textAlign:"center",transition:"all 0.2s"}}>
                    <div style={{fontSize:18,marginBottom:3}}>🎤</div>
                    <div style={{fontSize:10,fontWeight:800,color:hostMicOn?C.yellow:C.sub,textTransform:"uppercase",letterSpacing:1}}>Mic</div>
                    <div style={{fontSize:9,color:C.sub,marginTop:2}}>{hostMicOn?"On":"Off"}</div>
                  </button>
                )}
              </div>
